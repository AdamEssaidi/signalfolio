...
const DockIcon = ({ className, children }: DockIconProps) => {
  ...
  const magnifiedIconSize = Math.min(magnification * 0.5, MAX_ICON_SIZE);
  const iconSize = useSpring(
    useTransform(
      distanceCalc,
      [-distance, 0, distance],
      [BASE_ICON_SIZE, magnifiedIconSize, BASE_ICON_SIZE]
    ),
    SPRING
  );
  ...
  return (
    ...
    >
      <motion.div
        style={{
          width: shouldReduceMotion ? BASE_ICON_SIZE : iconSize,
          height: shouldReduceMotion ? BASE_ICON_SIZE : iconSize,
        }}
